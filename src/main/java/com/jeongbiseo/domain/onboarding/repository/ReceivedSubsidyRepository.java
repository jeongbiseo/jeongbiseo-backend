 ...
 * onboarding 리포지토리와 통일한 스타일 적응). setReceivedSubsidies(우선순위 4)가 쓰는 벌크 삭제를 더해 쓰기 경로를
 * 완성함(ONB-230 조회에 이어).
 */
public interface ReceivedSubsidyRepository extends JpaRepository<ReceivedSubsidy, Long> {
	...
	// 벌크 삭제(즉시 실행, H2). 파생 deleteBy는 flush가 이어지는 saveAll의 INSERT 뒤로 밀려 같은 목록 재PUT 시
	// UNIQUE 위반 500이 나므로, `@Modifying` 쿼리로 즉시 실행되게 함.
	@Modifying
