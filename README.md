...
> **`docs/`는 이 저장소에 없습니다.** API명세서·데이터모델·설계 원칙 등은 상위 umbrella 작업공간에 있고 git으로 추적되지 않아 **clone해도 따라오지 않습니다**(공유 경로는 팀에서 정하는 중). 저장소 안에서 바로 볼 수 있는 정본은 `AGENTS.md`와 `.agents/rules/`입니다.
